...
{ 
    ...
    posterior2.clear();
    posterior2.resize(SIZE, RealT(0));
                ...
                for (int p = i; p <= std::min(i+C_MAX_SINGLE_LENGTH,j); p++)
                {
                    ...
                    for (int q = j; q >= q_min; q--)
                    {
                        ...
                        if (p == i && q == j)
                        {
                            ...
                            posterior2[offset[p+1]+q] += Fast_Exp(outside + ScoreBasePair(i+1,j) + ScoreHelixStacking(i,j+1) + FCi[offset[p+1]+q-1]);
                        }
                        else
                        {
                            posterior[offset[p+1]+q] += Fast_Exp(outside + ScoreSingle(i,j,p,q) + FCi[offset[p+1]+q-1]);
                        ...
                        for (int q = j; q >= q_min; q--)
                        {
                            ...
                            posterior[offset[p+1]+q] +=
                                Fast_Exp(p == i && q == j ?
                                         ...
                                         ScoreJunctionB(q,p) + ScoreSingleNucleotides(i,j,p,q));
                            if (p == i && q == j)
                                posterior2[offset[p+1]+q] += Fast_Exp(score_helix + FCptr[q]);
                        }
                    }
                }
    ...
    for (int i = 1; i <= L; i++)
    {
        ...
        for (int j = i+1; j <= l; j++)
        {
            ...
            posterior2[offset[i]+j] = Clip(posterior2[offset[i]+j], RealT(0), RealT(1));
        }
    }
}
...
template<class RealT>
RealT *InferenceEngine<RealT>::GetPosterior2(const RealT posterior_cutoff) const
{
    RealT *ret = new RealT[SIZE];
    for (int i = 0; i < SIZE; i++)
...
template<class RealT>
RealT *InferenceEngine<RealT>::GetPosterior2(const RealT posterior_cutoff,
                                             std::vector<RealT>& p) const
{
    p.resize(SIZE);
    for (int i = 0; i < SIZE; i++)
